...
const MODEL_PRIORITY = ['gemini-2.5-flash', 'gemini-2.5-flash-lite', 'gemini-2.0-flash', 'gemini-2.0-flash-lite'];
...
- account: 계좌 또는 카드 정보 (은행명 + 계좌번호 그대로, 한국어 유지)
  * 카드 유효기간이 카드번호 주변에 어떤 형식(예: 08/26, 0826, (08/26))으로 오든 무조건 "(MM/YY)" 형식으로 정규화하여 포함하라.
  * 예시: "삼성카드 9420 8459 0169 9455 0826" → "삼성카드 9420 8459 0169 9455 (08/26)"
...
- memo: 고객메모 또는 특이사항
  ...
  * 계좌번호, 카드번호 등 account 필드에 이미 들어간 정보는 memo에 중복으로 넣지 마라.
  * 위 제외 규칙을 적용한 뒤 남는 내용이 없으면 memo는 null로 처리하라.
...
export async function parseWithGemini(systemPrompt: string, userText: string): Promise<string> {
  ...
  for (const modelName of MODEL_PRIORITY) {
    try {
      ...
      if (errMsg.includes('429') || errMsg.includes('Too Many Requests') || errMsg.includes('quota')) {
        console.warn(`[Gemini] ${modelName} 쿼터 초과 → 다음 모델로 전환`);
        continue;
      }
      throw err;
    }
  ...
  throw lastError || new Error('모든 Gemini 모델의 쿼터가 초과되었습니다.');
}
